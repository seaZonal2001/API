...
function findMatches(wordToMatch,citiesStates){
    return citiesStates.filter(cityState =>{
        const regX=new RegExp(wordToMatch,'ig');
...
function displayMatches(){
    ...
    const matchEl=findArray.map(place =>{
        const regX=new RegExp(this.value,'ig');
        const cityName=place.city.replace(regX,`<span class="highlight">${this.value}</span>`);
        const stateName=place.state.replace(regX,`<span class="highlight">${this.value}</span>`);
